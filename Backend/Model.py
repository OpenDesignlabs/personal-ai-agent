...
import os  # Import the os library for file path manipulation.
...
ChatHistory = [
    ...
    {"role": "User", "message": "what is today's date and by the way remind me that i have a dancing performance on role"},
...
def FirstLayerDMM(prompt: str = "test"):
    ...
    if any(word in prompt_lower for word in ['bye', 'exit', 'quit', 'goodbye']):
        return ["exit"]
    
    # Check for application commands
    app_keywords = ['open', 'close', 'launch', 'start', 'shutdown']
    if any(word in prompt_lower for word in app_keywords):
        if 'open' in prompt_lower:
            # Extract app name after 'open'
            if 'open ' in prompt_lower:
                app_name = prompt_lower.split('open ')[1].split(' and')[0].strip()
                return [f"open {app_name}"]
        elif 'close' in prompt_lower:
            if 'close ' in prompt_lower:
                app_name = prompt_lower.split('close ')[1].split(' and')[0].strip()
                return [f"close {app_name}"]
    
    # Check for play commands
    if 'play' in prompt_lower:
        if 'play ' in prompt_lower:
            song_name = prompt_lower.split('play ')[1].strip()
            return [f"play {song_name}"]
    
    # Check for search commands
    search_keywords = ['search', 'find', 'look for', 'google']
    if any(word in prompt_lower for word in search_keywords):
        return [f"google search {prompt}"]
